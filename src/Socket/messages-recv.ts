...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const localMsgRetryCache: CacheStore | undefined = shouldCloseMsgRetryCache ? (msgRetryCache) : undefined
	const localCallOfferCache: CacheStore | undefined = shouldCloseCallOfferCache ? (callOfferCache) : undefined
	const localPlaceholderResendCache: CacheStore | undefined = shouldClosePlaceholderResendCache ? (placeholderResendCache) : undefined
					...
					try {
						...
						const fullMessage = waproto.WebMessageInfo.fromObject({
							key: {
								...
								fromMe: areJidsSameUser(child.attrs.participant, authState.creds.me!.id)
							},
	...
	const handleGroupNotification = (
		...
		switch (child?.tag) {
			...
			case 'not_ephemeral':
				...
				break
			case 'modify':
				const oldNumber: string[] | undefined = getBinaryNodeChildren(child, 'participant')?.map(p => p.attrs?.jid).filter((jid): jid is string => !!jid)
			...
			case 'leave':
				...
				const participants: string[] | undefined = getBinaryNodeChildren(child, 'participant')?.map(p => p.attrs.jid)
				if (
	...
	const processNotification = async (node: BinaryNode) => {
		...
		switch (nodeType) {
			...
			case 'encrypt':
				...
				break
			case 'devices':
				const devices: BinaryNode[] | undefined = getBinaryNodeChildren(child, 'device')
			...
			case 'account_sync':
				if (child.tag === 'disappearing_mode') {
					...
					const blocklists: BinaryNode[] | undefined = getBinaryNodeChildren(child, 'item')
	...
	const handlePrivacyTokenNotification = async (node: BinaryNode) => {
		...
		const tokenNodes: BinaryNode[] | undefined = getBinaryNodeChildren(tokensNode, 'token')
	...
	const sendMessagesAgain = async (
		...
		const participantResolution: RetryParticipantResolution = resolveRetryParticipant(participant, remoteJid, retryNode)
		const sendToAll = !participantResolution.jid
		const hasDeviceIdentifier = Boolean(participantResolution.hasDevice)
		const shouldRecreateSession = false
		...
		await assertSessions([participantJidForSession], true)
	...
	const handleReceipt = async (node: BinaryNode) => {
		...
		if (Array.isArray(content)) {
			const items: BinaryNode[] | undefined = getBinaryNodeChildren(content[0], 'item')
					...
					async () => {
						...
						if (
							...
							if (isJidGroup(remoteJid) || isJidStatusBroadcast(remoteJid)) {
								if (attrs.participant) {
									const updateKey: keyof MessageUserReceipt = status === waproto.WebMessageInfo.Status.DELIVERY_ACK ? 'receiptTimestamp' : 'readTimestamp'
									ev.emit(
	...
	const handleMessage = async (node: BinaryNode) => {
		...
		if (msg.message?.protocolMessage?.type === waproto.Message.ProtocolMessage.Type.SHARE_PHONE_NUMBER && node.attrs.sender_pn) {
	...
	const fetchMessageHistory = async (
		...
		const pdoMessage = {
			...
			peerDataOperationRequestType: waproto.Message.PeerDataOperationRequestType.HISTORY_SYNC_ON_DEMAND
		}
