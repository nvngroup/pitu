...
export const makeNewsletterSocket = (config: SocketConfig) => {
	...
	return {
		...
		newsletterCreate: async(name: string, description?: string): Promise<NewsletterMetadata> => {
			...
			const rawResponse: NewsletterCreateResponse = await executeWMexQuery<NewsletterCreateResponse>(
				variables,
				QueryIds.CREATE,
		...
		newsletterMetadata: async(type: 'invite' | 'jid', key: string) => {
			...
			const result: unknown = await executeWMexQuery<unknown>(variables, QueryIds.METADATA, XWAPaths.xwa2_newsletter_metadata)
		...
		subscribeNewsletterUpdates: async(jid: string): Promise<{ duration: string } | null> => {
			...
			const liveUpdatesNode: BinaryNode | undefined = getBinaryNodeChild(result, 'live_updates')
			const duration: string | undefined = liveUpdatesNode?.attrs?.duration
