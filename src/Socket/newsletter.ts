...
const parseNewsletterCreateResponse = (response: NewsletterCreateResponse): NewsletterMetadata => {
	...
	return {
		...
		creationTime: parseInt(thread.creation_time, 10),
...
export const makeNewsletterSocket = (sock: GroupsSocket) => {
	...
	return {
		...
		newsletterMetadata: async(type: 'invite' | 'jid', key: string) => {
			const variables = {
				fetchCreationTime: true,
				fetchFullImage: true,
				fetchViewerMetadata: true,
				input: {
					key,
		...
		newsletterFollow: (jid: string) => {
			return executeWMexQuery({ newsletterId: jid }, QueryIds.FOLLOW, XWAPaths.xwa2_newsletter_follow)
		},

		newsletterUnfollow: (jid: string) => {
			return executeWMexQuery({ newsletterId: jid }, QueryIds.UNFOLLOW, XWAPaths.xwa2_newsletter_unfollow)
		},

		newsletterMute: (jid: string) => {
			return executeWMexQuery({ newsletterId: jid }, QueryIds.MUTE, XWAPaths.xwa2_newsletter_mute_v2)
		},

		newsletterUnmute: (jid: string) => {
			return executeWMexQuery({ newsletterId: jid }, QueryIds.UNMUTE, XWAPaths.xwa2_newsletter_unmute_v2)
		},
		...
		newsletterDemote: async(jid: string, userJid: string) => {
			await executeWMexQuery({ newsletterId: jid, userId: userJid }, QueryIds.DEMOTE, XWAPaths.xwa2_newsletter_demote)
		},

		newsletterDelete: async(jid: string) => {
			await executeWMexQuery({ newsletterId: jid }, QueryIds.DELETE, XWAPaths.xwa2_newsletter_delete_v2)
		}
	}
}
